...
export async function syncEconomyProjection() {
  ...
  await prisma.$transaction([
    prisma.economyItem.deleteMany(),
    ...items.reduce<Array<Prisma.PrismaPromise<unknown>>>((queries, _, index) => {
      if (index % 1_000 === 0) {
        queries.push(
          prisma.economyItem.createMany({
    ...
    prisma.inventoryProjectionMeta.update({
      data: {
        cs2LibVersion,
...
async function projectUserInventory(userId: string): Promise<ProjectionResult> {
  try {
    ...
    return await (async () => {
      ...
      const projection = await prisma.userInventoryProjection.findUniqueOrThrow({
        where: { userId }
      });
      if (user === null) {
        return "skipped";
      ...
      await prisma.userInventoryItem.deleteMany({ where: { userId } });
      ...
      if (inventory.stickers.length > 0) {
        await prisma.userInventoryItemSticker.createMany({
          data: inventory.stickers
        });
      }
      if (inventory.patches.length > 0) {
        await prisma.userInventoryItemPatch.createMany({ data: inventory.patches });
      }
      if (inventory.keychains.length > 0) {
        await prisma.userInventoryItemKeychain.createMany({
          data: inventory.keychains
        });
      }
      await prisma.userInventoryProjection.update({
        data: {
          failedSyncedAt: null,
